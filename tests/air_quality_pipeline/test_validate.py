"""Unit tests for air quality pipeline validation."""

from unittest.mock import MagicMock, patch

import pandas as pd

from air_quality_pipeline.validate import (
    get_air_quality_dataframe,
...
def test_validate_air_quality_data_success():
    ...
    with (
        patch("air_quality_pipeline.validate.get_ephemeral_context") as mock_context,
        patch("air_quality_pipeline.validate.save_validation_results") as mock_save,
        patch("air_quality_pipeline.validate.gx") as mock_gx,
    ):
        ...
        result = validate_air_quality_data(conn)

    assert result is True
    mock_save.assert_called_once_with(mock_result, "air_quality_validation")


def test_validate_air_quality_data_failure():
...
def test_get_air_quality_dataframe():
    """Test that get_air_quality_dataframe calls correct SQL."""
    conn = MagicMock()
    conn.execute.return_value.df.return_value = pd.DataFrame()
    get_air_quality_dataframe(conn)
    conn.execute.assert_called_once_with(
        "SELECT * FROM air_quality.air_quality_records"
    )


def test_validate_adds_correct_number_of_expectations():
    """Test that 7 expectations are added for air quality data."""
    conn = MagicMock()
    conn.execute.return_value.df.return_value = pd.DataFrame()
    ...
    with (
        patch("air_quality_pipeline.validate.get_ephemeral_context") as mock_context,
        patch("air_quality_pipeline.validate.save_validation_results"),
        patch("air_quality_pipeline.validate.gx") as mock_gx,
    ):
    ...
    assert mock_suite.add_expectation.call_count == 7
